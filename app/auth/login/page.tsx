    ...
    <div className="w-full max-w-[480px] bg-white border border-hairline p-6 md:p-8">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full border border-hairline px-4 py-3 focus:outline-none focus:border-ink" placeholder="you@example.com" />
        </div>
        <div>
          <label className="block text-sm text-ink mb-2">Password</label>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full border border-hairline px-4 py-3 focus:outline-none focus:border-ink" placeholder="••••••••" />
        </div>
        <div className="flex items-center justify-between text-sm">
          <label className="flex items-center gap-2"><input type="checkbox" className="w-4 h-4" /> <span className="text-ink-2">Remember me</span></label>
          <Link href="/auth/forgot-password" className="text-ink hover:underline">Forgot password?</Link>
        </div>
        <button type="submit" disabled={loading} className="w-full bg-ink text-white py-3 uppercase tracking-wider text-sm hover:bg-ink-2 transition-colors disabled:opacity-60">
          {loading ? 'Signing In...' : 'Sign In'}
        </button>
      </form>

      <p className="mt-6 text-center text-sm text-ink-2">
        Don&apos;t have an account? <Link href="/auth/register" className="text-ink font-medium hover:underline">Create Account</Link>
      </p>

      <div className="mt-6 pt-6 border-t border-hairline text-center">
        <p className="text-xs text-ink-2/60">Admin? Use admin@annaparis.com / admin123</p>
      </div>
    </div>
  )
}
